...
class PostController extends Controller {
	...
	public function getUpdatePost($post_id) {
		...
		$i =0;
		// create an array of all the current posts category ids.
		...
		// send to view: the post, all the categories, all the posts categories, all the posts categories ids.
	...
	public function postCreatePost(Request $request) {
		...
		if(strlen($request['categories']) > 0) {
			//create an array from our list of categories, then foreach category add to posts_categories table.
	...
	public function postUpdatePost(Request $request) {
		...
		// Categories update
		// remove all the current posts_categories data linked to post
		$post->categories()->detach();
		// Attach all the new ones.
		if(strlen($request['categories']) > 0) {
			//create an array from our list of categories, then foreach category add to posts_categories table.
			$categoryIDs = explode(',', $request['categories']);
			foreach ($categoryIDs as $categoryID) {
